...
- **2026-07-20**：桌面启动 Loading 定案——窗口边框出现即显示页面内唯一 Loading；进度条由 Python `_set_progress()` 按真实阶段推进到 100%，再加 `body.app-ready` 切完整主界面。避免 Tk/Win32 外置 splash、`hidden=True` 等 DOM ready 再 show、前端自行动画进度三类方案，分别会导致样式割裂、黑屏/Loading 太晚、两次 Loading/假进度。启动视觉问题先用 `策划工具箱.bat` 验证源码，用户确认后再打包。
- **2026-04-16**：记忆系统启用
...
### fix(cmp): 对比模式下文件预设配置未生效——子进程与前端读取不同配置文件
- **场景**：2026-06-01 SVN 对比模式下，为 "Texts" 文件配置的标题行（1）、ID列（1）、输出列（`::ID::,::SC::,SubstituteId`）未生效，输出格式不正确
- **根因**：打包 exe 后，前端保存配置到 `%APPDATA%/planning-toolbox/svn_gui_config.json`，但 `svn_oneclick_compare.py` 子进程的 `_load_cmp_file_settings()` 从自身脚本目录读取内置默认配置，两者不是同一个文件。从 `ae50a37`/`3b998dd` 引入 APPDATA 部署后开始存在
- **解决方案**：`_load_cmp_file_settings()` 优先检测 APPDATA 路径的配置文件，存在则使用，不存在回退到脚本目录
