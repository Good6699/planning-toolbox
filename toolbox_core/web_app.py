...
def _svn_cat_rev(svn_exe, url, rev, auth_args):
...
def _svn_get_prev_rev(svn_exe, url, rev, auth_args):
...
def _extract_text_diffs(src_bytes, prev_bytes, title_rows, id_col):
    """比较两个版本的 Excel bytes，返回变更行的行号列表。
    返回: changed_rows (list of int, 1-based row indices in src_bytes)"""
    import openpyxl
    try:
...
def _merge_texts_xlsm(source_url, target_path, file_path, file_revs,
                       svn_user, svn_pass, cfg, put):
    ...
    id_col_raw = cfg.get("cmp_id_col") or "1"
    try:
    ...
    for rev in sorted_revs:
        # 找到文件在该版本的真实前一版本
        prev_rev = _svn_get_prev_rev(svn, file_url, rev, auth_args)
        put(f"版本 {rev} (基准: {prev_rev}): 下载中...\n")

        cur_bytes = _svn_cat_rev(svn, file_url, rev, auth_args)
        prev_bytes = _svn_cat_rev(svn, file_url, prev_rev, auth_args)
        if not cur_bytes:
            put(f"  无法下载版本 {rev}，跳过\n")
            continue
        if not prev_bytes:
        ...
        # 提取差异行
        diffs = _extract_text_diffs(cur_bytes, prev_bytes, title_rows, id_col)
        if not diffs:
            put(f"  版本 {rev}: 无差异\n")
            continue
        ...
        for sheet_name, rows in by_sheet.items():
            ...
            if not ws_src or not ws_tgt:
                continue
            a, u, _ = _merge_sheet_rows(ws_src, ws_tgt, rows, title_rows, id_col, set(), put)
            added += a
            updated += u
...
def _merge_worker(task_id, source_url, target_path, revisions, rev_file_map, files,
    ...
    try:
        ...
        for f in files:
            ...
            is_texts = basename == "texts.xlsm"
            q.put(f"\n── 合并: {file_path} (版本: {file_revs}){' [单元格合并]' if is_texts else ''} ──\n")
            try:
                if is_texts:
                    m, s = _merge_texts_xlsm(
                        source_url, target_path, file_path, file_revs,
                        svn_user, svn_pass, cfg, _put)
                    total_merged += m
                    total_skipped += s
                else:
                    result = svn_merge(
