...
class UploadTabMixin:
    ...
    def _run_upload(self):
        ...
        def _do_copy():
            ...
            for iid, info in checked_items:
                ...
                try:
                        ...
                        if matched:
                            ...
                            for item in os.listdir(src_path):
                                ...
                                if os.path.isdir(s):
                                    ...
                                    copied_files.extend(_collect_copied(s, d))
                                else:
                            ...
                            copied_files.extend(_collect_copied(src_path, dest))
                    ...
                    self._ulog(f"   ❌ [{name}] 复制失败: {e}", "error")
                    fail += 1
        ...
        _th.Thread(target=_do_copy, daemon=True).start()

    def _check_svn_and_confirm(self, target_dir, copied_files):
        """复制完成后，直接执行 svn add + TortoiseSVN 提交"""
        if not copied_files:
            ...
            try:
                ...
                try:
                    ...
                    if r_cl.returncode == 0:
                        modified_now = set(to_add + to_commit)
                        cl_clean = []
